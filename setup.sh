...
download_files() {
  local base_url="https://github.com/dashqee/DHQ-XKeen-UI/releases"
  local download_url="$base_url/latest/download"
  local bin_name="dhqclash-router-$ARCH"
...
install_xkeenui() {
  if [[ -d $STATIC_DIR || -f $XKEENUI_BIN || -f $XKEENUI_INIT || -f $LIGHTTPD_CONF ]]; then
    printf "${YELLOW}\n Обнаружены файлы DHQClash Router, запуск переустановки...\n${NC}"
    uninstall_xkeenui
  fi
  ...
  $XKEENUI_INIT start &>/dev/null &
  if ! spinner $! "Запуск DHQClash Router..."; then
    printf "${RED_BOLD}\n Не удалось запустить DHQClash Router.${NCN}"
    exit 1
  fi
...
update_xkeenui() {
  [ -f "$XKEENUI_BIN" ] || { printf "${ERROR} Ошибка: DHQClash Router не установлен!${NCN}"; exit 1; }
  ...
  if [ ! -f $XKEENUI_INIT ]; then
    ...
    spinner $! "Остановка DHQClash Router..."
  else
    sed -i 's|^PROCS=/opt/sbin/xkeen-ui$|PROCS=xkeen-ui|' /opt/etc/init.d/S99xkeen-ui
  fi
...
uninstall_xkeenui() {
  printf "\n Данное действие ${RED_BOLD}удалит${NC} DHQClash Router, его файлы и зависимости.\n\n"
  ...
  spinner $! "Удаление файлов DHQClash Router..."
  printf "${SUCCESS} Удаление DHQClash Router завершено${NCN}"
}
...
finish_setup() {
  ...
  printf "${SUCCESS} DHQClash Router успешно $1!${NCN}"
...
legacy_installation_check() {
  if [ -f "$LIGHTTPD_CONF" ]; then
    ...
    rm -f "$LIGHTTPD_CONF"
    printf "${YELLOW}\n Веб-сервер lighttpd для работы DHQClash Router более не используется.\n${NC}"
...
printf "${CYAN}"
cat <<'EOF'
  ____  __  ______  ________           __
 / __ \/ / / / __ \/ ____/ /___ _____/ /_
/ / / / /_/ / / / / /   / / __ `/ __  /
/ /_/ / __  / /_/ / /___/ / /_/ / /_/ /
\___\_\/ /_/_____/\____/_/\__,_/\__,_/
                    Router
EOF
